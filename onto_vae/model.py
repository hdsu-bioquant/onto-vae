...
###-------------------------------------------------------------###
##           VAE WITH ONTOLOGY IN ENCODER OR DECODER             ##
###-------------------------------------------------------------###
...
class OntoVAE(nn.Module):
    ...
    def __init__(self, onto, in_features, layer_dims_enc, layer_dims_dec, mask_list, neuronnum=1, drop=0, z_drop=0.5):
        super(OntoVAE, self).__init__()

        self.onto = onto
        self.in_features = in_features
        self.layer_dims_enc = list(np.array(layer_dims_enc) * neuronnum)
        self.layer_dims_dec = layer_dims_dec
        self.mask_list = mask_list
        if self.onto == 'encoder':
    ...
    def vae_loss(self, reconstruction, mu, log_var, data, kl_coeff):
        #rec_loss = F.mse_loss(reconstruction, data, reduction='sum')/data.shape[0]
        #kl_loss = torch.mean(-0.5 *  torch.sum(1 + log_var - mu.pow(2) - log_var.exp(), dim=1), dim=0)
